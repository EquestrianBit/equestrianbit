...
uint256 hashGenesisBlock("0x6329acdf52f320f600c64ae63e01351545a9064964fd352008970805d74e14b4");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // EquestrianBit: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "EquestrianBit Signed Message:\n";
...
{
    // EquestrianBit: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    // Subsidy is cut in half every 132000 blocks, which will occur approximately every 3 months
    nSubsidy >>= (nHeight / 132000); // EquestrianBit: 132k blocks in 3 months

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 4 * 60 * 60; // retarget every 4 hours
static const int64 nTargetSpacing = 60; // block time 1 minute
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // EquestrianBit: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // EquestrianBit: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // EquestrianBit: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x4483bc04b80b60f12db8aedfac19036311a821c568ed110e8eb54dd0d868fe1b");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        block.nVersion = 1;
        block.nTime    = 1388109424;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 1580062;

        if (fTestNet)
        {
            block.nTime    = 1388089289;
            block.nNonce   = 1564793;
        }
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // EquestrianBit: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("EquestrianbitMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("EquestrianbitMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static EquestrianbitMiner(CWallet *pwallet)
{
    printf("EquestrianbitMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("equestrianbit-miner");
    ...
    try { loop {
        ...
        printf("Running EquestrianbitMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("EquestrianbitMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&EquestrianbitMiner, pwallet));
}
